...
namespace Marius.Pinta.Script.Parser
{
    public class JavaScriptParser
    {
        ...
        private bool _disableAutomaticSemicolonInsertion;
        ...
        public JavaScriptParser(bool strict, bool disableAutomaticSemicolonInsertion)
        {
            _strict = strict;
            _disableAutomaticSemicolonInsertion = disableAutomaticSemicolonInsertion;
        }
        ...
        private Token ScanNumericLiteral()
        {
            ...
            if (!isDouble)
            {
                ...
                var longValue = 0L;
                if (long.TryParse(number, NumberStyles.AllowLeadingSign, CultureInfo.InvariantCulture, out longValue))
                {
                    return new Token
                    {
                        Type = Tokens.NumericLiteral,
            ...
            try
            {
                doubleValue = double.Parse(number, NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent, CultureInfo.InvariantCulture);
            ...
            {
                doubleValue = number.Trim().StartsWith("-") ? double.NegativeInfinity : double.PositiveInfinity;
            }
            catch (Exception)
            {
        ...
        private void ConsumeSemicolon()
        {
            ...
            var line = _lineNumber;
            SkipComment();
            if (!_disableAutomaticSemicolonInsertion && _lineNumber != line)
            {
                return;
            }
            ...
            if (_disableAutomaticSemicolonInsertion || _lookahead.Type != Tokens.EOF && !Match("}"))
            {
        ...
        private Expression ParsePostfixExpression()
        {
            ...
            if (_lookahead.Type == Tokens.Punctuator)
            {
                if ((Match("++") || Match("--")) && (!PeekLineTerminator() || _disableAutomaticSemicolonInsertion))
                {
                    // 11.3.1, 11.3.2
        ...
        private Statement ParseContinueStatement()
        {
            ...
            if (PeekLineTerminator() && !_disableAutomaticSemicolonInsertion)
            {
                if (!_state.InIteration)
                {
                    ThrowError(Token.Empty, Messages.IllegalContinue);
